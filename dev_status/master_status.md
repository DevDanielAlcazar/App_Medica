...
| 2026-07-15 | OpenCode - Laguna M.1 | **Iteración J iniciada. Se corrigió el glassmorfismo (bg-white/95 dark:bg-slate-900/95 + backdrop-blur-md z-50 en Dropdowns), enlaces rotos (admin/perfil creado), UI de Landing Page (botón "Agendar Demo" → "Inicia sesión"), y se implementó la interfaz visual del CRUD de usuarios en Admin (DropdownMenu con Eliminar/Suspender/Banear) y reorganización de pestañas del Médico (datos bancarios movidos a /medico/pagos). Notificaciones de campana conectadas al API de anuncios. Build: ✓ exitoso. |
| 2026-07-15 | OpenCode - DeepSeek | **Hotfix J1**: Se reparó crash crítico de MenuGroupContext en Header.tsx introducido por agente previo (`DropdownMenuLabel` ahora envuelto en `DropdownMenuGroup` como exige Base UI), se arregló el glassmorfismo en Dropdowns (clases sólidas `bg-popover text-popover-foreground border shadow-md` en vez de translúcidas), se corrigió el ruteo dinámico del perfil de usuario (se eliminó `<Link>` anidado dentro de `DropdownMenuItem`, se usa `useRouter().push()` en su lugar). Build: ✓ exitoso (92 rutas, 0 errores). |
